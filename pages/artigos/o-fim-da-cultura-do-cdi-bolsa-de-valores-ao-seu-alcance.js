                ...
                <div className="post type-post status-publish format-standard has-post-thumbnail hentry category-crazy tag-food tag-inspiration">
                  <div className="single_section_content box blog_large_post_style">
                    ...
                    <div className="post_content">
                      <p>
                      De alguns anos para cá a cultura do <a uk-tooltip={openDict('cdi')} className={`${artigoStyles.dictLink}`}>CDI</a> vem chegando ao seu fim. A taxa de juros referencial <a uk-tooltip={openDict('selic')} className={`${artigoStyles.dictLink}`}>SELIC</a> está no nível mais baixo da história: 2,25% ao ano (valor atualizado em 17 de junho de 2020). Entre julho de 2015 e agosto de 2016, esta taxa estava em elevados 14,25% ao ano.
                      </p>
                      <p>
                      Em 2015, um investimento em <a uk-tooltip={openDict('tesouro selic')} className={`${artigoStyles.dictLink}`}>Tesouro SELIC</a> rendendo 100% do CDI ao ano (14,25%), poderia ser considerado um ótimo negócio. Era um rendimento de alta liquidez, alto rendimento e baixo risco.
                      </p>
                      <p>
                      ...
                      <p>
                        <img className="wp-image-4861 alignleft" src={post.images[1]} alt="" width="338" height="423" />A economia brasileira sempre foi indexada e atrelada à taxa de juros do CDI. O rendimento das aplicações financeiras tem como <a uk-tooltip={openDict('benchmark')} className={`${artigoStyles.dictLink}`}>benchmark</a> princiapal o CDI. Por exemplo, <a uk-tooltip={openDict('cdb')} className={`${artigoStyles.dictLink}`}>CDBs</a> e títulos públicos sempre tiveram o seu desempenho medido em comparação com o percentual do <a uk-tooltip={openDict('cdi')} className={`${artigoStyles.dictLink}`}>CDI</a>.
                      </p>                        
                      <p>
                      O fim da cultura do CDI quer dizer que o dinheiro aplicado no banco em juros pós-fixados agora rende muito menos do que antes. Atualmente, para buscar maiores rendimentos, o investidor precisa aumentar sua exposição a investimentos de maiores riscos, e passar a investir na bolsa de valores. E, neste cenário, é fundamental aprender a investir em <a uk-tooltip={openDict('ação')} className={`${artigoStyles.dictLink}`}>Ações</a> e em <a uk-tooltip={openDict('Fundos Imobiliários/FII')} className={`${artigoStyles.dictLink}`}>FIIs</a>.
                      </p>
                      <p>

                ...
                <div className="post type-post status-publish format-standard has-post-thumbnail hentry category-crazy tag-food tag-inspiration">
                  <div className="single_section_content box blog_large_post_style">
                    ...
                    <div className="post_content">
                      ...
                      <p>Economista por formação, mentor de Warren Buffett e considerado o precursor da estratégia de investimento denominada <a uk-tooltip={openDict('buy and hold')} className={`${artigoStyles.dictLink}`}>Buy and Hold</a>. Graham é considerado um dos maiores investidores de todos os tempos.</p>
                      ...
                      <p>Aos 25 ele já possuía grande prestígio atuando em um emprego em <a uk-tooltip={openDict('wall street')} className={`${artigoStyles.dictLink}`}>Wall Street</a>. Porém, em mais um revés na sua vida, sente a falência novamente na pele, perdendo quase todos os seus <a uk-tooltip={openDict('investimento')} className={`${artigoStyles.dictLink}`}>investimentos</a> durante a <a uk-tooltip={openDict('crise de 1929')} className={`${artigoStyles.dictLink}`}>crise de 1929</a>.</p>
                      <p>Como lição tirada após esse momento e após se reerguer, escreve junto a David Dodd o livro que hoje é considerado um dos mais importantes sobre <a uk-tooltip={openDict('análise fundamentalista')} className={`${artigoStyles.dictLink}`}>Análise Fundamentalista</a>, intitulado Security Analysis.</p>
                      <blockquote>
                      ...
                      <p>Porém, sua maior obra seria lançada em 1949. O Investidor Inteligente é leitura fundamental para investidores do <a uk-tooltip={openDict('value investing')} className={`${artigoStyles.dictLink}`}>Value Investing</a>. Nesta obra, Graham cunha o método onde deve-se analisar os <a uk-tooltip={openDict('balanço patrimonial')} className={`${artigoStyles.dictLink}`}>balanços patrimoniais</a> e o <a uk-tooltip={openDict('fluxo de caixa')} className={`${artigoStyles.dictLink}`}>fluxo de caixa</a> da empresa para saber se a sua <a uk-tooltip={openDict('ação')} className={`${artigoStyles.dictLink}`}>ação</a> estava cara ou barata.</p>
                      <p>
                        <img className="wp-image-4861 alignleft" src={post.images[1]} alt="" width="338" height="423" />
                      </p>
                      <p>Estes princípios buscam evitar perdas e auxiliam o investidor a ter uma maior disciplina emocional.</p>
                      <p>Investindo em ações que possuem <a uk-tooltip={openDict('cotação')} className={`${artigoStyles.dictLink}`}>cotações</a> menores que o seu <a uk-tooltip={openDict('valor intrínseco')} className={`${artigoStyles.dictLink}`}>valor intrínseco</a>, ou seja, com maior <a uk-tooltip={openDict('margem de segurança')} className={`${artigoStyles.dictLink}`}>margem de segurança</a>, tendem a manter o investidor mais tranquilo do que aqueles que compraram ações nos topos pensando que aquele papel iria se valorizar ainda mais.</p>
                      <p className={`${artigoStyles.subtitle}`}>Lição principal</p>
                      <p>Além da margem de segurança, Graham considerava ações como negócios e não como  <a uk-tooltip={openDict('ativo')} className={`${artigoStyles.dictLink}`}>ativos</a> <a uk-tooltip={openDict('especulação financeira')} className={`${artigoStyles.dictLink}`}>especulativos</a>.</p>
